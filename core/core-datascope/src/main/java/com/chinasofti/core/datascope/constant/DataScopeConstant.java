package com.chinasofti.core.datascope.constant;
...
public interface DataScopeConstant {
	...
	String DEFAULE_USERID_COLUMN = "create_user";
	/**
	 * 获取部门数据
	 */
	//String DATA_BY_DEPT = "select id from sys_dept where ancestors like concat(concat('%', ?),'%') and is_deleted = 0";

	String DATA_BY_DEPT = "select id from sys_dept where parent_id = ? and is_deleted = 0";
	...
	static String dataByMapper(int size) {
		return "select resource_code, scope_column, scope_field, scope_type, scope_value from sys_scope_data where scope_class = ? and id in (select scope_id from sys_role_scope where scope_category = 1 and role_id in (" + buildHolder(size) + "))";
	}
