...
import com.chinasofti.core.tool.utils.ReflectUtil;
	...
	private void resolveEntity(T entity) {
		...
		if (entity.getId() == null) {
			...
			if (entity.getStatus() == null) {
				entity.setStatus(BootConstant.DB_STATUS_NORMAL);
			}
			entity.setCreateTime(now);
			
			if( entity.getIsDeleted() == null )
			{
				entity.setIsDeleted(BootConstant.DB_NOT_DELETED);
			}
		} else if (user != null) {
			// 处理修改逻辑
			entity.setUpdateUser(user.getUserId());
		...
		Field field = ReflectUtil.getField(entity.getClass(), BootConstant.DB_TENANT_KEY);
		if (ObjectUtil.isNotEmpty(field)) {
			Method getTenantId = ClassUtil.getMethod(entity.getClass(), BootConstant.DB_TENANT_KEY_GET_METHOD);
			String tenantId = String.valueOf(getTenantId.invoke(entity));
			if (ObjectUtil.isEmpty(tenantId)) {
				Method setTenantId = ClassUtil.getMethod(entity.getClass(), BootConstant.DB_TENANT_KEY_SET_METHOD, String.class);
